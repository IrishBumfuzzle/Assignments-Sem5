...
# Ablations: C5 fixed-patch control, vocab-8000 tokenization ablation (old
# official C1-C4), the 15-epoch vocab-512 probe — everything but checkpoints
if [ -d "outputs/ablations" ]; then
  find "outputs/ablations" -type f ! -name "*.pt" -exec cp --parents {} "$STAGE/" \;
fi
