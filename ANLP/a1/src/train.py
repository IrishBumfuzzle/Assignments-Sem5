...
def parse_args():
    ...
    p.add_argument("--patching", choices=["entropy", "fixed"], default="fixed",
                   help="C5 patching scheme: fixed = stride --patch-size "
                        "(default, official C5), entropy = entropy-based dynamic "
                        "patching (explored variant)")
...
def _log_patch_stats(wandb_run, patch_info: dict):
    ...
    for split in ("train", "val", "test"):
        ...
        counts = st.get("hist_counts")
        if counts:
            edges = st.get("hist_edges")
            if edges is None or len(edges) != len(counts) + 1:
                # old checkpoints stored rounded centers only; the bins were
                # always [0.5, 1.5, ..., n+0.5] for n = len(counts)
                edges = np.arange(0.5, len(counts) + 1.5, dtype=np.float64)
            log[f"c5/{split}_patch_size_hist"] = wandb.Histogram(
                np_histogram=(np.array(counts, dtype=np.float64),
