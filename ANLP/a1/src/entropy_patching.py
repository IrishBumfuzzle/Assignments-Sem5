...
def patch_stats(line_lengths: List[int],
    ...
    total_bytes = sum(line_lengths)
    hist, bin_edges = np.histogram(lens, bins=np.arange(0.5, lens.max() + 1.5)) \
        if lens.size else (np.zeros(1), np.array([0.5, 1.5]))
    return {
        ...
        # exact float bin edges (n+1 of them); wandb.Histogram needs these
        "hist_edges": [float(e) for e in bin_edges],
    }
